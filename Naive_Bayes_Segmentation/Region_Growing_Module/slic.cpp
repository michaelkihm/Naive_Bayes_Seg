...
void Slic::init_data(Mat *image) {
    ...
    for (int i = step; i < image->cols - step/2; i += step) {
        for (int j = step; j < image->rows - step/2; j += step) {
            ...
            Scalar colour(image->at<Vec3b>(nc.y, nc.x));
...
Point Slic::find_local_minimum(Mat *image, Point center) {
    ...
    for (int i = center.x-1; i < center.x+2; i++) {
        for (int j = center.y-1; j < center.y+2; j++) {
            //Scalar c1 = cvGet2D(image, j+1, i);
            //Scalar c2 = cvGet2D(image, j, i+1);
            //Scalar c3 = cvGet2D(image, j, i);
            
            Scalar c1(image->at<Vec3b>(j+1,i));
            Scalar c2(image->at<Vec3b>(j,i+1));
            Scalar c3(image->at<Vec3b>(j,i));
    ...
    for (int i = 0; i < NR_ITERATIONS; i++) {
            ...
            for (int k = centers[j][3] - step; k < centers[j][3] + step; k++) {
                for (int l = centers[j][4] - step; l < centers[j][4] + step; l++) {
                    ...
                    if (k >= 0 && k < image->cols && l >= 0 && l < image->rows) {
                        Scalar colour(image->at<Vec3b>( l, k));
        ...
        for (int j = 0; j < image->cols; j++) {
            for (int k = 0; k < image->rows; k++) {
                ...
                if (c_id != -1) {
                    Scalar colour(image->at<Vec3b> (k, j));
...
void Slic::display_center_grid(Mat *image, Scalar colour) {
    for (int i = 0; i < (int) centers.size(); i++) {
        //Circle(image, Point(centers[i][3], centers[i][4]), 2, colour, 2);
...
void Slic::display_contours(Mat *image, Scalar colour) {
    ...
    for (int i = 0; i < (int)contours.size(); i++) {
        image->at<Vec3b>(contours[i].y, contours[i].x)[0] = colour[0];
        image->at<Vec3b>(contours[i].y, contours[i].x)[2] = colour[2];
        image->at<Vec3b>(contours[i].y, contours[i].x)[1] = colour[1];
    }
}
...
void Slic::colour_with_cluster_means(Mat *image) {
    ...
    for (int i = 0; i < image->cols; i++) {
        for (int j = 0; j < image->rows; j++) {
            ...
            image->at<Vec3b>(j, i)[0] = ncolour[0];
            image->at<Vec3b>(j, i)[1] = ncolour[1];
            image->at<Vec3b>(j, i)[2] = ncolour[2];
        }
    }
}
